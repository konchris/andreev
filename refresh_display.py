...
def refresh_display():
        ...
        try:
            ...
            try:
                ...
                _sample_res= _self.data["agilent_voltage_voltage"][-1]/_self.data["agilent_current_voltage"][-1]
                _self.ui.editIVTimeEstimate.setText("%i s"%(round(_delay*(abs(_high-_low)/_steps)))) 
                _self.ui.editIVMinEstimate.setText("%f mV"%(_sample_res * _low * 1e3))
                _self.ui.editIVMaxEstimate.setText("%f mV"%(_sample_res * _high * 1e3))
                _self.ui.editIVStepsEstimate.setText("%f uV"%(_sample_res * _steps * 1e6))
            except Exception,e:
                log("IV Time",e)
            ...
            if interval < 100:
                interval = 100
            _self.timer_second.setInterval(interval)
