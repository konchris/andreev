...
base_path = r"D:\Desktop\150314_Pb314_Switching06"   # custom
total_path = [os.path.join(base_path,'db_%i.h5'%(db))]
config_path = os.path.join(base_path,'config.txt')
measurement_switching = "switching_1426339545.txt"
...
if True:
    ...
    sw_file = open(os.path.join(base_path, measurement_switching),"r")
    
    x_high,y_high = [],[]
    x_low,y_low = [],[]
    for row in sw_file.readlines():
        a = row.split(",")
        if len(a) == 5:
            
            
            if a[1] == "high":
                x_high.append(int(a[0]))
                x_high.append(int(a[0]))
                y_high.append(float(a[2]))
                y_high.append(float(a[3]))
            if a[1] == "low":
                x_low.append(int(a[0])+0.5)
                x_low.append(int(a[0])+0.5)
                y_low.append(float(a[2]))
                y_low.append(float(a[3]))
    
    
    sw_plot = pl.figure(figsize=(8,6),dpi=120)
    sw_plot.subplots_adjust(hspace = 0.4, wspace = 0.25)
    
    length = len(x_high)
    plot_rows = 5    
    for i in range(plot_rows):
        sw_sub = sw_plot.add_subplot(plot_rows,1,i+1)
           
        start,end=length/plot_rows*i,length/plot_rows*(i+1)
        #print start,end
        sw_sub.plot(x_high[start:end],y_high[start:end],"^r",label="High Pulse")
        sw_sub.plot(x_low[start:end],y_low[start:end],"*b",label="Low Pulse")
        
        sw_sub.set_xlim(start/2.0,end/2.0)
        c = y_low + y_high
        _max = max(c)*1.1
        _min = min(c)*0.8
        sw_sub.set_ylim([_min,_max])
        
        sw_sub.set_ylabel("G/$G_0$")
        sw_sub.set_xlabel("Cycles")
        #sw_sub.set_title("Switching Cycles (%i cycles)"%(len(x_high)))
    #sw_sub.legend()

    sw_plot.suptitle("Switching Cycles (%i cycles)"%(len(x_high)/2))
    sw_plot.savefig(os.path.join(base_path,"switching.png"))
